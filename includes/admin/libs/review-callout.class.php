...
class YVIL_Review_Callout{
	 ...
	 * @param number $delay
	 */
	public function __construct( $option_name, YVIL_Message $message, YVIL_User $user, $delay = 7 ){
...
class YVIL_Message{
	...
	/**
	 * Saves the YVIL_User instance needed to display message actions
	 * 
	 * @param YVIL_User $user
	 */
	public function set_user( YVIL_User $user ){
		$this->user = $user;
	}
...
<?php
	...
	private function note_footer( $echo = true ){
		...
		$links = array( 
				sprintf( $template, $this->review_url, __( "Sure, I'd love to!", 'yt-video-importer-lite' ) ),
				sprintf( $template, esc_url( add_query_arg( $this->user->get_query_arg( 'yes' ) ) ), __( 'No, thanks.', 'yt-video-importer-lite' ) ),
				sprintf( $template, esc_url( add_query_arg( $this->user->get_query_arg( 'yes' ) ) ), __( "I've already given a review.", 'yt-video-importer-lite' ) ),
				sprintf( $template, esc_url( add_query_arg( $this->user->get_query_arg( 'later' ) ) ), __( 'Ask me later.', 'yt-video-importer-lite' ) )
		);
